...
import { AMBIENT_T, computeAshraeMetrics, type SceneAshraeMetrics, T_MAX_REC } from "./lib/ashrae";
...
function pueFor(scene: SceneGraph): { pue: number; itKw: number; coolKw: number } {
  const itKw =
    scene.furniture.filter((f) => f.heatOutput > 0).reduce((s, f) => s + f.heatOutput, 0) *
    PEAK_LOAD;
  const coolKw = coolingEnergyBase[scene.id] ?? 0;
...
function renderTimeSeries(scene: SceneGraph) {
  ...
  const peakT = peakTempBase[scene.id] ?? 32;
